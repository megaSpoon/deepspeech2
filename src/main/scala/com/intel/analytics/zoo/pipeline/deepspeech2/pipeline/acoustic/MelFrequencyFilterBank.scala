...
class MelFrequencyFilterBank ( override val uid: String)
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val convert = udf { (samples: mutable.WrappedArray[Double]) =>
      ...
      val mm6 = if (isDefined(uttLength)) {
        if($(uttLength) > mm3.rows) {
          val mm4 = DenseMatrix.zeros[Double](($(uttLength) - mm3.rows), mm3.cols)
  ...
  private def createFilterBank(numFilter: Int, fftsz: Int, sampleFreqHz: Int): DenseMatrix[Double] = {
    ...
    val fbank = DenseMatrix.zeros[Double](numFreqs, numFilter)

    for (j <- 0 until numFilter) {
      bins += Math.floor(scaleby * mel_to_hz(minMelFreq + (j + 2) * melFreqDelta)).toInt
      var i = bins(j)
      while( i < bins(j + 1)) {
        fbank(i, j) = (i - bins(j)).toDouble / (1.0F * (bins(j + 1) - bins(j)))
        i += 1
      }
