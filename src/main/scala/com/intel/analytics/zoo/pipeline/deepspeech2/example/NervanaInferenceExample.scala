...
object NervanaInferenceExample {
  ...
  def main(args: Array[String]): Unit = {
    parser.parser.parse(args, LocalOptimizerPerfParam()).foreach { param =>
      ...
      print("samples type" + samples.getClass())
      val samplesDF = Seq((audioPath, samples.map(_.toDouble))).toDF("path", "samples")
      ...
      val modelTransformer = new DeepSpeech2NervanaModelTransformer().setInputCol("features").setOutputCol("prob")
