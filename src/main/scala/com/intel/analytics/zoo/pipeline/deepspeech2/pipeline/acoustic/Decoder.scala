...
/**
  ...
  */
abstract class Decoder( alphabet: String,
               blankIndex: Int,
  ...
  def getSoftmax(prop:  Array[Array[Double]]): Array[Array[Double]] = {

    val columns = prop.transpose
    columns.map { col =>
      val max = col.max
      val sum = col.map(t => math.exp(t - max).toDouble).sum
      col.map(t => math.exp(t - max).toDouble / sum)
    }.transpose
  }
  ...
  /**
    ...
    */
  def decode(probs: Array[Array[Double]]): String
}
